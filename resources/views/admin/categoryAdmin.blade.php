...
<div class="container">
    ...
    <div class="row ">
     ...
     <h2 class="text-center fw-bold">الفئه</h2>
     <div class="container">
     <a href="{{route("categorys.create")}}" class="btn btn-info mb-3">اضافه فئه</a>
     <table class="table table-bordered border-primary table-hover">
        ...
        <tbody>
            ...
            <tr class="text-center">
                <th>{{$data->id}}</th>
                <td>{{$data->name}}</td>
                <td><a href="{{route("categorys.show",[$data->id])}}" class="btn btn-success">Show</a></td>
                <td><a href="{{route("categorys.edit",[$data->id])}}" class="btn btn-primary">Edit</a></td>
                 <td>
                     <form action="{{route("categorys.destroy",[$data->id])}}" method="post">
                        @method('DELETE')
                        @csrf
